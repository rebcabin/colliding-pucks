...
/*  Chrysalis, the SUN workstations and the Mark 3 hypercube.	*/
...
#ifdef MARK3_OR_SUN_OR_BF_MACH
    stat = (int) fgets ( buff, size, fp );

/* note cast to int for stat.  if using BF_PLUS there is a stat++ so
made cast instead of redefining stat to be a pointer. */
...
#ifdef BF_PLUS
/* Alas ! The Butterfly+  does not have fflush */
#endif
