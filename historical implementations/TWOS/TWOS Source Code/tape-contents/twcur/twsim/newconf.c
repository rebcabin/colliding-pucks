...
#endif
{
	...
	if (argv[1] [0]== '-') 
	  ...
	  {
	    yXyin = fopen(argv[1], "r");
#else
	  {
	    yXyin = HOST_fopen(cnfg_file, "r");
#endif
	    if(yXyin == (FILE *)NULL)
	     {
...
/* yXyparse will exit after running this at top level. Switch the
files for yXyin and tell objend not to really exit when it is called. 
Then go back to yXyparse on the next line which is in new file. */
