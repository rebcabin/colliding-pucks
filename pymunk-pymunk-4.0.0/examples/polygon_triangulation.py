...
while not(quit):
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            pygame.image.save(g_screen, "polygon_triangulation.png")
    ...
    if len(clicked_points) == 1:
        pygame.draw.circle(g_screen, (150, 0, 0), clicked_points[0], 10, 0)
    if len(clicked_points) > 1:
        pygame.draw.lines(g_screen, (150, 0, 0), True, clicked_points, 20)
      
    # draw triangles
    if len(poly.triangles) > 0:
        for triangle in poly.triangles:
            pygame.draw.lines(g_screen, (0, 0, 200), True, triangle, 12)
         
    # draw hulls
    if len(poly.convexes) > 0:
        for convex in poly.convexes:
            pygame.draw.lines(g_screen, (255, 255, 255), True, convex, 2)
