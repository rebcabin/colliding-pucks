...
while running:
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            pygame.image.save(g_screen, "flipper.png")
    ...
    ### Clear screen
    g_screen.fill(THECOLORS["white"])
    
    ### Draw stuff
    for ball in balls:
        p = to_pygame(ball.body.position)
        pygame.draw.circle(g_screen, THECOLORS["blue"], p, int(ball.radius), 2)
    ...
    for line in static_lines:
        ...
        p1 = to_pygame(pv1)
        p2 = to_pygame(pv2)
        pygame.draw.lines(g_screen, THECOLORS["lightgray"], False, [p1, p2])
    ...
    for f in [r_flipper_shape, l_flipper_shape]:
        ...
        color = THECOLORS["red"]
        pygame.draw.lines(g_screen, color, False, ps)
